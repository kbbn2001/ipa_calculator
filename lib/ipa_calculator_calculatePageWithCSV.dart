...
class _CalculatePageWithCSVStatus extends State<CalculatePageWithCSV> {
  ...
  Future<List<double>> processCsv() async {
    ...
    String filePath = join(appDocDir.path, 'backdata2.csv');

    var myData = await rootBundle.loadString("assets/excelFile/backdata2.csv");
    List<List<dynamic>> rowsAsListOfValues = const CsvToListConverter().convert(myData);

    //final int purifyTemperature = int.parse(purifyTemperatureController!.text); //1도 단위일때 사용
    final int purifyTemperature = (double.parse(purifyTemperatureController!.text) * 10).round(); //0.1도 단위일때 사용
    final double purifyMoisture = double.parse(purifyMoistureController!.text);
    //final int newTemperature = int.parse(newTemperatureController!.text); //1도 단위일때 사용
    final int newTemperature = (double.parse(newTemperatureController!.text) * 10).round(); //0.1도 단위일때 사용
